...
// CommonHMAC and CommonDigestSPI are SPI not exposed in the public iOS SDK.
// We skip the per-page digest validation in sharedCacheIsValid() because the
// cache we operate on lives on the user's own device — integrity is implied.
// See WP11_SKIP_DIGEST_VALIDATION below.
#define WP11_SKIP_DIGEST_VALIDATION 1
...
#if WP11_SKIP_DIGEST_VALIDATION
    // (Skipped — see top of file. We trust the on-device dsc.)
...
#if WP11_SKIP_DIGEST_VALIDATION
            (void)dscDigestFormat; (void)cdHashBuffer; (void)fileOffset;
#else
