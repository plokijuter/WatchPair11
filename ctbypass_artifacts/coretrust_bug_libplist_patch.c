...
#ifdef USE_LIBPLIST
    // libplist-based replacement for CoreFoundation plist generation
    plist_t cdhashes_array = plist_new_array();
    plist_array_append_item(cdhashes_array, plist_new_data((const char *)sha1CDHash, CC_SHA1_DIGEST_LENGTH));
    plist_array_append_item(cdhashes_array, plist_new_data((const char *)sha256CDHash, CC_SHA1_DIGEST_LENGTH));
    plist_t cdHashesDict = plist_new_dict();
    plist_dict_set_item(cdHashesDict, "cdhashes", cdhashes_array);

    char *xml_buf = NULL;
    uint32_t xml_len = 0;
    plist_to_xml(cdHashesDict, &xml_buf, &xml_len);
    plist_free(cdHashesDict);
    if (!xml_buf) {
        printf("Failed to encode CDHashes plist\n");
        return -1;
    }

    // Add text CDHashes attribute
    if (!CMS_signed_add1_attr_by_txt(newSigner, "1.2.840.113635.100.9.1", V_ASN1_OCTET_STRING, (const uint8_t *)xml_buf, xml_len)) {
